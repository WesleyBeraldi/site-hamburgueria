...
- Os scripts versionados em `database/create_db` documentam a criação limpa da estrutura MySQL.
...
4. Defina um caminho absoluto e persistente para `UPLOADS_PATH`.
...
- Mantenha cópias operacionais do MySQL pela infraestrutura escolhida e teste a restauração em ambiente isolado. Os arquivos em `database/create_db` recriam uma instalação limpa, não substituem uma cópia dos dados reais de produção.
