import { ArrowRight, QrCode, ShieldCheck } from 'lucide-react';
import { useState } from 'react';
import { Link, useNavigate, useParams } from 'react-router-dom';

import { useApp } from '../../context/appContext';
import styles from './garcom.module.css';

function AcessoGarcom() {
  const { token } = useParams();
  const { funcionarios, entrarGarcom } = useApp();
  const navigate = useNavigate();
  const [pin, setPin] = useState('');
  const [erro, setErro] = useState('');
  const funcionario = funcionarios.find((item) => item.token === token && item.status === 'Ativo');
  const ativos = funcionarios.filter((item) => item.status === 'Ativo');

  function entrar(event) {
    event.preventDefault();
    if (!entrarGarcom(token, pin)) {
      setErro('PIN incorreto. Confirme o código e tente novamente.');
      return;
    }
    navigate('/garcom/mesas');
  }

  return (
    <main className={styles.acessoPagina}>
      <section className={styles.acessoCard}>
        <div className={styles.iconeAcesso}>{token ? <ShieldCheck size={30} /> : <QrCode size={30} />}</div>
        <h1>{funcionario ? 'Funcionário identificado' : 'Acesso do garçom'}</h1>
        <p>{funcionario ? 'Digite seu PIN pessoal para entrar no atendimento.' : 'Escaneie seu QR Code individual para identificar o acesso.'}</p>

        {funcionario ? (
          <form className={styles.formAcesso} onSubmit={entrar}>
            <div className={styles.identificado}><span>{funcionario.nome.charAt(0)}</span><div><strong>{funcionario.nome}</strong><small>{funcionario.cargo}</small></div></div>
            <div className={styles.campo}><label htmlFor="pinGarcom">PIN de acesso</label><input id="pinGarcom" type="password" inputMode="numeric" maxLength="6" autoFocus value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, ''))} placeholder="Digite seu PIN" /></div>
            {erro && <div className={styles.erro}>{erro}</div>}
            <button type="submit" className={styles.botaoPrincipal}>Entrar no atendimento <ArrowRight size={17} /></button>
          </form>
        ) : (
          <div className={styles.linksDemo}>
            {token && <div className={styles.erro}>Este QR Code é inválido ou o funcionário está inativo.</div>}
            {ativos.map((item) => <Link className={styles.linkDemo} key={item.id} to={`/garcom/acesso/${item.token}`}><div><strong>{item.nome}</strong><small>{item.cargo}</small></div><ArrowRight size={17} /></Link>)}
            <Link className={styles.linkDemo} to="/"><div><strong>Voltar ao site</strong><small>Página inicial da hamburgueria</small></div></Link>
          </div>
        )}
      </section>
    </main>
  );
}

export default AcessoGarcom;
