  ...
  return (
    <AdminLayout titulo="Categorias" subtitulo="Organize as seções do cardápio sem valores fixos no código." acao={acao}>
      ...
      <section className={styles.card}>
        <div className={styles.topoCard}><div><h2>Categorias do cardápio</h2><p>Categorias inativas e seus produtos deixam de aparecer ao cliente.</p></div></div>
        <div className={styles.listaAdicionaisAdmin}>
          {categorias.map((categoria) => (
            <div className={styles.adicionalLinha} key={categoria.id}>
              <div><strong>{categoria.nome}</strong><span>Ordem {categoria.ordem} • {produtos.filter((produto) => produto.categoriaId === categoria.id).length} produtos</span></div>
